...
BEGIN
    IF EXISTS (SELECT saldo FROM tbl_rekening WHERE norek = Pnorek order by created_date desc LIMIT 1) THEN
        UPDATE tbl_rekening
        SET saldo = vLast + Pgram, update_date = extract(epoch from now())
        WHERE norek = Pnorek;

        INSERT INTO tbl_topup 
        SELECT Pid,Pgram,harga_topup, harga_buyback,Pnorek,vLast + Pgram 
    ...
    ELSE
        ...
        INSERT INTO tbl_topup 
        SELECT Pid,Pgram,harga_topup, harga_buyback,Pnorek,Pgram
        FROM tbl_harga ORDER BY created_date DESC LIMIT 1;
    END IF;
END;
$$ LANGUAGE plpgsql;
